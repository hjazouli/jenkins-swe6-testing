...
def run_certification():
    ...
    while len(samples) < 3 and (time.time() - timeout_start < 5):
        ...
        if "[BCM" in line:
            ...
            print(
                f"   ↳ Sample {len(samples)}: Received at {time.strftime('%H:%M:%S')}"
            )
    ...
    if len(samples) >= 2:
        ...
        if 0.9 <= diff <= 1.1:
            print("   [PASS] Timing is stable.")
        else:
            print("   [WARN] Timing jitter detected.")
    else:
    ...
    if success_count == total_cmds:
        print("   [PASS] UART RX Interrupt is solid.")
    else:
    ...
    if found_reset:
        print("   [PASS] Soft-Reset logic verified.")
    else:
